...
class AgodaAdapterConfig:
    ...
    reservations_endpoint: EndpointSpec = field(
        default_factory=lambda: EndpointSpec(
            ...
            method="GET",
            response_paths=("data.items", "data.reservations", "items", "reservations"),
        )
    )
    changes_endpoint: EndpointSpec = field(
        default_factory=lambda: EndpointSpec(
            path="/bookings/changes",
            method="GET",
            response_paths=("data.items", "data.changes", "items", "changes"),
        )
    )
    cancelled_endpoint: EndpointSpec = field(
        default_factory=lambda: EndpointSpec(
            ...
            method="GET",
            response_paths=("data.items", "data.cancelled", "items", "cancelled"),
        )
    )
    inventory_endpoint: EndpointSpec = field(
        default_factory=lambda: EndpointSpec(
            path="/inventory",
            method="GET",
            response_paths=("data.rooms", "data.inventory", "rooms", "inventory"),
        )
    )
...
class AgodaAdapter(OTAAdapter):
    ...
    def __init__(self, session: requests.Session, config: AgodaAdapterConfig) -> None:
        ...
        self._impl = HttpJsonOTAAdapter(
            ...
            reservation_fields=ReservationFieldSpec(
                reservation_id=("reservationId", "bookingId", "booking_id", "id"),
                room_type=("roomTypeName", "roomType", "room_name", "roomName"),
                checkin=("checkInDate", "checkIn", "arrivalDate", "checkin"),
                checkout=("checkOutDate", "checkOut", "departureDate", "checkout"),
                nights=("nights", "lengthOfStay", "stayNights"),
                status=("bookingStatus", "reservationStatus", "status"),
            ),
            inventory_fields=InventoryFieldSpec(
                provider_item_id=("roomTypeId", "roomId", "id", "providerItemId"),
                date=("date", "day", "stayDate"),
                stock=("availability", "available", "stock", "remaining"),
            ),
        )
